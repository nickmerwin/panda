...
#      elsif /Duration: N\/A|bitrate: N\/A/im.match(@raw_response)
    ...
    def fps
      ...
      # First look for fps      
      match = /([0-9\.]+) fps/.match(video_stream)
      return match[1] unless match.nil?
      
      # First look for tbr
      match = /([0-9\.]+) tbr/.match(video_stream)
      return match[1] unless match.nil?
      
      # Finally fall back to stream frame error
      match = /Seems stream \d+ codec frame rate differs from container frame rate: \d+.\d+ \(\d+\/\d+\) -> (\d+.\d+) \(\d+\/\d+\)/.match(@raw_response)
      return match[1] unless match.nil?
      nil
